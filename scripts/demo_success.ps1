...
$tools = @(Invoke-OpenCodeApi -Method Get -Path "/tools")
$toolNames = @($tools | ForEach-Object { $_.name })
foreach ($requiredTool in @("list_files", "read_file", "search_text", "summarize_repo")) {
    Assert-Condition ($toolNames -contains $requiredTool) "Missing API tool: $requiredTool"
}
...
$created = Invoke-OpenCodeApi -Method Post -Path "/tasks" -Body @{
    ...
    question = "Fix calculator.add so the fixture test passes."
...
Write-Host "Analysis status: $($analysis.status)"

Write-Step "Submitting the passing unified diff"
$unifiedDiff = @'
--- a/calculator.py
+++ b/calculator.py
@@ -1,2 +1,2 @@
...
$proposal = Invoke-OpenCodeApi -Method Post -Path "/tasks/$taskId/patches" -Body @{
    ...
    reason = "Correct calculator.add and verify the fixture test."
    risk_level = "LOW"
    target_files = @("calculator.py")
    generated_by = "demo_success"
}
Assert-Condition ($proposal.validation_status -ceq "VALID") "Patch is not VALID."

Write-Step "Reviewing the exact diff and binding approval to its SHA-256"
...
Assert-Condition (
    -not [string]::IsNullOrWhiteSpace([string]$approved.approved_command_hash)
) "Approval did not bind a command hash."
Assert-Condition ($null -ne $approved.approved_task_revision) "Approval lacks task revision."

Write-Step "Executing the approved patch and fixture tests"
$execution = Invoke-OpenCodeApi -Method Post -Path "/tasks/$taskId/execute"
Assert-Condition ($execution.status -ceq "SUCCEEDED") "Task did not reach SUCCEEDED."
$report = $execution.execution_report
Assert-Condition ($report.tests_passed -eq $true) "ExecutionReport tests_passed is false."
Assert-Condition ($report.rollback_triggered -eq $false) "Unexpected rollback."
Assert-Condition ($report.source_unchanged -eq $true) "Source integrity check failed."
Assert-Condition ($report.final_status -ceq "SUCCEEDED") "Report status mismatch."
Assert-Condition (
    $report.final_manifest_hash -ceq $report.expected_manifest_hash
) "Final workspace manifest does not match the approved result."
Assert-Condition (@($report.modified_files).Count -eq 1) "Unexpected modified-file count."
Assert-Condition (
    @($report.modified_files)[0] -ceq "calculator.py"
) "Unexpected modified file."
Assert-Condition (
    @($report.test_results).Count -eq 1 -and @($report.test_results)[0].exit_code -eq 0
) "Fixture test did not exit successfully."

$finalTask = Invoke-OpenCodeApi -Method Get -Path "/tasks/$taskId"
$finalDiff = Invoke-OpenCodeApi -Method Get -Path "/tasks/$taskId/diff"
$logs = @(Invoke-OpenCodeApi -Method Get -Path "/tasks/$taskId/logs")
Assert-Condition ($finalDiff.validation_status -ceq "APPLIED") "Patch is not APPLIED."
$workspaceCalculator = Join-Path ([string]$finalTask.workspace_path) "calculator.py"
Assert-Condition (Test-Path -LiteralPath $workspaceCalculator -PathType Leaf) (
    "Workspace calculator.py is missing."
)
Assert-Condition (
    ([System.IO.File]::ReadAllText($workspaceCalculator) -match "return left \+ right")
) "The retained workspace does not contain the approved fix."
...
Write-Host "`nSUCCESS DEMO PASSED: approved workspace edit succeeded; source fixture unchanged." `
